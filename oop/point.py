class Point:
    ...
    # 생성자 -> 여러 가지 생성자에 대응하는 단 하나의 생성자를 만들어야 함
    # 주로 객체의 초기화 담당
    ...
    # 우리가 만든 새 자료형에 대한 연산자의 작동 방식을 작성할 필요가 있다.
    def __add__(self, other):
        # Point(x,y) + other
        if isinstance(other, (int, float)):     # other의 타입 체크 ; int, float 형인지 체크
...
# 부모가 가진 메서드를 재정의한다.
class ColorPoint(Point):        # 부모가 Point인 서브 클래스
